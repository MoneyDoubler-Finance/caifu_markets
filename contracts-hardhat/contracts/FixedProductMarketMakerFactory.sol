...
// File @gnosis.pm/util-contracts/contracts/ConstructedCloneFactory.sol@v3.0.0-alpha.3
...
contract FixedProductMarketMakerFactory is ConstructedCloneFactory, FixedProductMarketMakerData {
    ...
    IERC20 public allowedCollateralToken;
    address public owner;
    uint public defaultFee;

    uint public constant MAX_FEE = 5 * (10 ** 16); // 5% (scaled by 1e18 in FPMM)

    event AllowedCollateralUpdated(address indexed previousCollateral, address indexed newCollateral);
    event DefaultFeeUpdated(uint previousFee, uint newFee);
    event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);

    modifier onlyOwner() {
        require(msg.sender == owner, "FPMMFactory: not owner");
        _;
    }

    constructor(IERC20 _allowedCollateralToken, uint _defaultFee) public {
        require(address(_allowedCollateralToken) != address(0), "FPMMFactory: invalid collateral");
        require(_defaultFee <= MAX_FEE, "FPMMFactory: fee too high");

        implementationMaster = new FixedProductMarketMaker();
        allowedCollateralToken = _allowedCollateralToken;
    ...
    function transferOwnership(address newOwner) external onlyOwner {
        require(newOwner != address(0), "FPMMFactory: zero owner");
        emit OwnershipTransferred(owner, newOwner);
        owner = newOwner;
    }

    function setAllowedCollateralToken(IERC20 newCollateral) external onlyOwner {
        require(address(newCollateral) != address(0), "FPMMFactory: invalid collateral");
        emit AllowedCollateralUpdated(address(allowedCollateralToken), address(newCollateral));
        allowedCollateralToken = newCollateral;
    }

    function setDefaultFee(uint newFee) external onlyOwner {
        require(newFee <= MAX_FEE, "FPMMFactory: fee too high");
        emit DefaultFeeUpdated(defaultFee, newFee);
        defaultFee = newFee;
    }
    ...
    {
        require(address(allowedCollateralToken) != address(0), "FPMMFactory: collateral not configured");
        if (address(_collateralToken) != address(0)) {
            require(_collateralToken == allowedCollateralToken, "FPMMFactory: collateral mismatch");
        }

        uint appliedFee = fee;
        if (appliedFee == 0) {
            appliedFee = defaultFee;
        } else {
            require(appliedFee == defaultFee, "FPMMFactory: fee mismatch");
        }
